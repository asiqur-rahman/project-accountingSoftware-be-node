...
const Tables = () => {
    ...
    const InActiveCurrentUser= async () => {
        setDeleteModalShow(false);
        console.log(userId)
        if(userId>0){
            setLoader(true);
            await Axios.post('/user/inactive/'+userId)
            .then(function (response) {
                setLoader(false);
                setUserId(0);
                history.push("/user/index/inactive");
            })
                        ...
                        {props => (
                            <div>
                                <Row>
                                    <Col xl={8}>
                                        ...
                                        <div className="btn-group ml-1">
                                            <Link className="btn btn-primary" to="/user/index/active" >Active</Link>
                                            <Link className="btn btn-white" to="/user/index/inactive" >InActive</Link>
                                        </div>
                                    </Col>
                                    <Col xl={4}>
                                        <Row>
                                ...
                                <BootstrapTable
                                    {...props.baseProps}
                                    bordered={false}
                                    pagination={paginationFactory({ sizePerPage: 5, sizePerPageRenderer: sizePerPageRenderer, sizePerPageList: [{ text: '5', value: 5, }, { text: '10', value: 10 }, { text: '25', value: 25 }, { text: userList.length, value: userList.length }] })}
    ...
    return (
        <>
            ...
            <Modal show={deleteModalShow} onHide={()=>setDeleteModalShow(false)}>
                ...
                <Modal.Footer>
                <Button variant="primary" onClick={()=>InActiveCurrentUser()}>
                    Confirm
                </Button>
