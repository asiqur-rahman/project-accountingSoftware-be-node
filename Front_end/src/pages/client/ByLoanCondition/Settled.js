...
const Tables = () => {
    ...
    const GetCommissionFormat=(cell, row)=>{
        return (
            ...
            {roleCode==2? 
                <div>
                    {row.Commission==0?
                    <button className="btn btn-outline-warning btn-sm" size="sm" onClick={()=>{updateCommissionStatus(row.Id);}}>
                        Non Paid
                    </button>
                    :
             ...
             :row.Commission==0?
                <span className={`badge badge-soft-warning py-1`}>Non paid</span>
                :
                <span className={`badge badge-soft-info py-1`}>Paid</span>}
            </div>
            );
    }
    ...
    const GetClients = async () => {
        setActiveBtn(5);
        setLoader(true);
        setRoleCode(authUtils.getRoleCode());
        await Axios.get('/client/byLoanCondition/'+6)
    ...
    const updateCommissionStatus = async (loanId) => {
        setLoader(true);
        await Axios.post('/client/updateCommissionStatus',{
            LoanId:loanId
          })
        .then(function (response) {
                                ...
                                <Row>
                                    ...
                                    <Link className="btn btn-danger mb-sm-0" to="/client/create" ><i className="uil-plus mr-1"></i>Create</Link>
                                        ...
                                        <Link className="btn btn-white" to="/client/ByLoanCondition/processing" >Processing</Link>
                                            ...
                                            <Link className="btn btn-primary" to="/client/ByLoanCondition/settled" >Settled</Link>
                                        </div>
                                    </Col>
                                    <Col xl={4}>
                                        <Row>
            ...
            <Row className="page-title">
                <Col md={12}>
                    <PageTitle
                        ...
                        title={'Clients - Settled Loan Stage'}
                    />
                </Col>
            </Row>
